...
def main():
    ...
    df_test_stu = torch.load(os.path.join(processed_dir, f"{app_name}.df.pt"))
    test_loader_stu = torch.load(os.path.join(processed_dir, f"{app_name}.test.pt"))

    teacher_models = [select_tch(option) for option in tch_options]
    teacher_models_paths = [os.path.join(model_dir, f"{app_name}.teacher.{option}.pth") for i, option in enumerate(tch_options)]
    ...
    model_save_path = os.path.join(model_dir, f"{app_name}.student.{option}.{alphas[0]*100}.pth")
    tsv_path = os.path.join(results_dir, f"{app_name}.student.{option}.{alphas[0]*100}.tsv")
