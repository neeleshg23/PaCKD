...
def main():
    ...
    if app_name == 'bc-3':
        teacher_models = [sys.argv[3], sys.argv[4]]
    else:
        teacher_models = [sys.argv[3], sys.argv[4], sys.argv[5]]
    ...
    for tch, option in enumerate(teacher_models, start=1):
        ...
        model_save_path = os.path.join(model_dir, cluster_option, f"{app_name}.teacher_{tch}.{option}.pth")
        
        test_loader = torch.load(os.path.join(processed_dir, cluster_option, f"{app_name}.test_{tch}.pt"))
        df_test = torch.load(os.path.join(processed_dir, cluster_option, f"{app_name}.df_{tch}.pt"))
        ...
        res_path = os.path.join(results_dir, cluster_option, f"{app_name}.teacher_{tch}.{option}.res.tsv")
